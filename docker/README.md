...
> 💡 **请用 Chrome 浏览器打开（重要）**：Docker 版在群晖/威联通等系统上**不会生成桌面图标**，只能用**浏览器**访问。实测：**Chrome（电脑/手机都行）一切正常**；但**部分手机自带浏览器（如荣耀内置浏览器）回放录像会黑屏**（只有进度条、没画面）——遇到就换 **Chrome**。
> 📱 手机上把页面「添加到主屏幕」，就能像 App 一样一键打开。
...
**原因**：群晖「控制面板 → 安全性 → 防火墙」默认只放行局域网网段（如 `192.168.1.0/24`），**Docker 用的 `172.17.x.x` 网段不在放行范围 → 被全部丢弃**。
...
好处：① 不用改群晖防火墙；② 容器里能看到 NAS 的真实 IP，启动日志直接显示 `http://192.168.1.100:8091`（不再是 `0.0.0.0`）。
...
**Q：手机上回放录像黑屏 / 只有进度条没有画面？**
换 **Chrome 浏览器**打开即可。部分手机自带浏览器（实测荣耀自带浏览器）对回放的 mp4 支持不好；直播走 HLS 所以能看，回放走原生 mp4 就会黑屏。电脑上的 Chrome 完全正常。
