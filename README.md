...
- **运行身份**：目前以 `root` 运行（原因与「最小权限」计划见下方「安全与隐私」）
...
- **运行身份**：目前以 `root` 运行 —— 因为需要往**你自己选的任意目录**写录像、并读 `/proc` 采样资源占用
  ...
  - ⏳ **最小权限（Roadmap）**：飞牛支持让应用以专属低权限用户运行（`run-as: package`），系统已为本应用建好 `fn-hiknvr` 用户；切换时要配套改「录像目录权限方案」（低权限用户只能写自己有权写的目录，否则会录不了像），因此安排在 **P3** 与打包一起做
