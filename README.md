...
面向 **海康威视（Hikvision）** 摄像头的 **飞牛 fnOS 本地监控套件**：实时预览、连续 + 事件双轨录像、网页回放、本地移动侦测。
纯本地运行，**只读拉取 RTSP**，数据只存在你自己的 NAS 上。

> 前身是一个自用脚本，现在已经产品化：摄像机参数、录像目录都可在应用内配置，支持多台摄像机。
...
- 一台 **海康威视摄像头**（或海康 NVR 的某个通道），且已开启 RTSP
...
> ⚠️ 摄像头最大并发 RTSP 会话数一般是 **3 路**。本应用每台摄像机占 **2 路**（主码流 1 + 子码流 1），所以不要把同一台摄像头添加两次。
...
- 连续录像**按小时分目录**，跨小时会自动切目录；每段默认 **5 分钟**（`segmentSeconds` 可调）
- **保留天数**默认 3 天，过期自动删除（`retentionDays` 可调）
- 文件用普通 mp4（`+faststart`），手机直接可播
- 磁盘写满会自动停止录像并在界面提示，不会删你的其它数据
...
**Q：加了 2 台以上后不显示 / 卡顿？**
检查是不是同一台摄像头被加了两次（会超并发上限）；或把「预览码流」改成子码流。
...
- 默认通过飞牛的**统一网关**访问（需飞牛登录态）；局域网端口 8091/8444 可选开启
- 以 `root` 运行（需要读 `/proc` 做资源采样、写录像目录）；权限最小化在计划中
